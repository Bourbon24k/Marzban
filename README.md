...
Форк основан на Marzban `v0.8.4` и добавляет несколько практичных патчей для продакшен-использования:

- XHTTP GET-uplink поля сохраняются в подписках и v2ray-json конфигах.
- Ноды стабильнее проходят health-check благодаря увеличенным таймаутам и обработке API timeout.
- Истёкшие пользователи получают понятное уведомление в клиенте вместо нерабочих серверов.
- Для пользователей можно задавать лимит устройств по HWID из запроса подписки.
- В дашборде добавлены лимит устройств и список зарегистрированных устройств пользователя.
- Есть API для просмотра и удаления устройств пользователя.

Подробности реализации: [PATCHES.md](./PATCHES.md).
