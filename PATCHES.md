# Custom Marzban patches

Форк [Gozargah/Marzban](https://github.com/Gozargah/Marzban) `v0.8.4` с патчами для:

1. **XHTTP GET-uplink через CDN/proxy**
2. **Стабильность нод** при медленных ответах API
3. **Уведомление истёкшим подписчикам** прямо в клиенте
4. **Лимит устройств по HWID**

База ветки — тег `v0.8.4` (оригиналы из развёрнутого образа `gozargah/marzban:latest`
2025-01-09 байт-в-байт совпадают с тегом).

Готовый образ: `ghcr.io/bourbon24k/marzban:0.8.4-custom`.
...
| `app/db/migrations/.../custom0001_device_limit.py` | миграция БД |
| `app/routers/user.py` | API устройств |
| `app/dashboard/...` | поле «Лимит устройств» + просмотр/удаление устройств |

---

## Патч 1 — CDN/proxy XHTTP GET-uplink (`v2ray.py` + `config.py`)

### Проблема
CDN/proxy — кэширующий L7-прокси. Он режет `POST`-uplink (отвечает `405`), а mainline-XHTTP
шлёт uplink через POST. Решение — `uplinkHTTPMethod: GET`
...
- Константы перед `generate_subscription`:
  ...
  EXPIRED_NOTICE_LINES = [
      ...
      "➡️ Продлите: t.me/custom_vpn_bot",   # ← здесь меняется текст/ссылка
  ]
  ```
...
## Патч 4 — лимит устройств по HWID
...
### Что добавлено
**БД** (`models.py` + миграция `custom0001device`):
...
docker build -t marzban-custom .

# или использовать готовый образ (если опубликован в GHCR)
docker pull ghcr.io/bourbon24k/marzban:0.8.4-custom
```
